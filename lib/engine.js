...
const postProcessAnswers = (answers) => {
  ...
  if (clone.wfType === 'Domain' && !clone.hostControllerCount) {
    clone.hostControllerCount = 1;
  }
  if (clone.wfType === 'Domain' && !clone.domainScenario) {
    clone.domainScenario = 'simple';
  }
  if (!clone.hawkflyWfEap) {
    clone.hawkflyWfEap = 'WildFly';
  }
  if (!clone.cassandraCount) {
    clone.cassandraCount = 1;
  }
  if (!clone.cassandraVersion) {
