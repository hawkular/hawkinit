...
const printAnswers = (answers) => {
  let json;
  if (!answers.defaultCredentials) {
    const origPassword = answers.hawkularPassword;
    answers.hawkularPassword = answers.hawkularPassword.replace(/./g, '*');
    json = JSON.stringify(answers, null, '  ');
    answers.hawkularPassword = origPassword;
  } else {
    json = JSON.stringify(answers, null, '  ');
  }
  console.log(`Answers:\n${chalk.grey(json)}`);
...
const postProcessAnswers = (answers) => {
  ...
  // sort by keys
  const ordered = {};
  Object.keys(clone).sort().forEach((key) => ordered[key] = clone[key]);
  return ordered;
};
