...
const runItInternal = (answers, directory, timeout) => {
  if (timeout) {
    ...
    dc.kill(directory, timeout);
    // dc.cassandraCleanup(answers.cassandraVersion, timeout);
  }
  process.on('exit', () => {
    process.on('exit', () => {
  ...
  if (answers.cassandraCount > 1) {
    ...
    process.on('exit', () => {
      ...
      dc.cassandraCleanup(0, answers.cassandraVersion);
    });
...
const show = (save, timeout) => {
  ...
  return inquirer.prompt(questions).then((answers) => {
    if (save) {
      ...
      fs.writeFileSync(path.join(__dirname, 'answers.json'), JSON.stringify(answers, null, '  '));
    }
    runIt(answers, timeout);
  });
};
